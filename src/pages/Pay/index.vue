...
import { reqOrderInfo, reqCheckPayStatus } from "@/api";
export default {
  name: "Pay",
  data() {
    return {
    ...
    async payNow() {
      ...
      this.$alert(`<img src=${url} />`, "请你微信支付", {
        ...
        beforeClose: (type, instance, done) => {
          console.log(type, this, instance);
      ...
      if (!this.timer) {
        this.timer = setInterval(async () => {
          let result = await reqCheckPayStatus(this.orderInfo.orderId);
          if (result.code == 200) {
            clearInterval(this.timer);
            this.timer = null;
            //保存状态码
